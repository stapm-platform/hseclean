...
Do the setup for imputation by carrying out all the data cleaning functions. There has already been some filling-in of missing data done by these cleaning functions - using simple rules, 
...
The function `select_data()` has the option to filter the data to retain only complete cases for certain variables e.g. the user might wish to retain only records with full information on age, sex, smoking status and the survey design variables.   
