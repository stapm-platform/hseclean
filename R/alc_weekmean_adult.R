...
alc_weekmean_adult <- function(
  ...
  # check abv data is used correctly ----- abv_data_2022 has updated abv and values from Kantar and cider is now a separate category.
  #### SHOULD WE APPLY THIS TO ALL? ####
  if (year %in% year_set4 & country == "England") {
    abv_data <- hseclean::abv_data_2022
    volume_data <- hseclean::alc_volume_data_2022
  }
  ...
  year_set4 <- 2022:2023 ## HSE cider separation
  ...
  if (year %in% c(year_set1, year_set2, year_set4) | country %in% c("Scotland")) {
    ...
    if (country == "England" & year %in% c(year_set4)) {
  ...
  if (year %in% c(year_set1, year_set2) | country %in% c("Scotland", "Wales")) {
    ...
    # half pints
    ## NSW recorded in pints rather than half-pints after 2019-20 & HSE from 2019 onwwards also
    if (country == "Wales" & year > 2019 | country == "England" & year >= 2019) {
    ...
    # if the respondent specifically answered "don't know" to the question on amount drunk,
  ...
  #################################
  # Normal Cider (from 2022 onwards)

  if (year %in% c(year_set4) & country == "England") {
    data[, vol_ncider := 0]

    # Pints (2022+ records in pints)
    ...
    data[ncidm1 == 1 & ncida == -8, vol_ncider := NA]
    data[ncidm2 == 1 & ncidb == -8, vol_ncider := NA]
    data[ncidm3 == 1 & ncidc == -8, vol_ncider := NA]
    data[ncidm4 == 1 & ncidd == -8, vol_ncider := NA]
  } else {
    data[, vol_ncider := 0]
  }

  #################################
  # Strong Cider (from 2022 onwards)
  ...
  if (year %in% c(year_set4) & country == "England") {
    ...
    # Handle "don't know" responses
    data[scidm1 == 1 & scida == -8, vol_scider := NA]
    data[scidm2 == 1 & scidb == -8, vol_scider := NA]
    data[scidm3 == 1 & scidc == -8, vol_scider := NA]
    data[scidm4 == 1 & scidd == -8, vol_scider := NA]
  } else {
    data[, vol_scider := 0]
  }
  ...
  if (year %in% c(year_set2, year_set4) & country == "England" | country == "Wales") {
    data[, vol_wine := 0]
  ...
  if (year %in% c(year_set2, year_set4) | country %in% c("Scotland", "Wales")) {
    data[, vol_pops := 0]
  ...
  # interview questions
  # if(year %in% c(year_set1, year_set2) | country %in% c("Scotland","Wales")) {
  if ((year %in% c(year_set1, year_set2, year_set4) & country == "England") | country %in% c("Scotland")) {
    data[, scnbeer := hseclean::alc_drink_freq(scnbeer)] # normal beer
    data[, scsbeer := hseclean::alc_drink_freq(scsbeer)] # strong beer
    data[, scspirit := hseclean::alc_drink_freq(scspirit)] # spirits
    data[, scsherry := hseclean::alc_drink_freq(scsherry)] # sherry
    data[, scwine := hseclean::alc_drink_freq(scwine)] # wine
    data[, scpops := hseclean::alc_drink_freq(scpops)] # alcopops/rtds
  }

  if ((year %in% c(year_set2, year_set4) & country == "England") | country == "Scotland") {
    # Normal beer
    data[, vol_scnbeer := 0]
    ...
    ### add btvol
    if (year >= 2019 & country == "England") {
      data[!is.na(scnbeeq4) & scnbeeq4 > 0, vol_scnbeer := vol_scnbeer + scnbeeq4 * volume_data[beverage == "nbeerbtlvol", volume]]
    }
    ...
    if (year >= 2019 & country == "England") {
      data[!is.na(scsbeeq4) & scsbeeq4 > 0, vol_scsbeer := vol_scsbeer + scsbeeq4 * volume_data[beverage == "sbeerbtlvol", volume]]
    }
  ...
  if (year %in% c(year_set4) & country == "England") {
    # Normal cider
    data[, vol_scncider := 0]
    data[!is.na(scncidq1) & scncidq1 > 0, vol_scncider := scncidq1 * volume_data[beverage == "nciderhalfvol", volume] * 2]
    data[!is.na(scncidq2) & scncidq2 > 0, vol_scncider := vol_scncider + scncidq2 * volume_data[beverage == "nciderscanvol", volume]]
    data[!is.na(scncidq3) & scncidq3 > 0, vol_scncider := vol_scncider + scncidq3 * volume_data[beverage == "nciderlcanvol", volume]]
    data[!is.na(scncidq4) & scncidq4 > 0, vol_scncider := vol_scncider + scncidq4 * volume_data[beverage == "nciderbtlvol", volume]]
    data[scncidq1 == -8, vol_scncider := NA]
    data[scncidq2 == -8, vol_scncider := NA]
    data[scncidq3 == -8, vol_scncider := NA]
  }
  ...
  if (year %in% c(year_set4) & country == "England") {
    data[, vol_scscider := 0]
    data[!is.na(scscidq1) & scscidq1 > 0, vol_scscider := scscidq1 * volume_data[beverage == "nciderhalfvol", volume] * 2]
    data[!is.na(scscidq2) & scscidq2 > 0, vol_scscider := vol_scscider + scscidq2 * volume_data[beverage == "nciderscanvol", volume]]
    data[!is.na(scscidq3) & scscidq3 > 0, vol_scscider := vol_scscider + scscidq3 * volume_data[beverage == "nciderlcanvol", volume]]
    data[!is.na(scscidq4) & scscidq4 > 0, vol_scscider := vol_scscider + scscidq4 * volume_data[beverage == "nciderbtlvol", volume]]
    data[scscidq1 == -8, vol_scscider := NA]
    data[scscidq2 == -8, vol_scscider := NA]
    data[scscidq3 == -8, vol_scscider := NA]
  }
  ...
  if (country == "England" & year >= 2019) {
    ...
    data[, scwineq3 := 0]
    data[, scwineq4 := 0]
    data[scwineq5 == 1 & !is.na(cwineqbt) & cwineqbt > 0, vol_scwine := cwineqbt * volume_data[beverage == "winebtlvol", volume]]
    data[scwineq6 == 1 & !is.na(cwineqgl) & cwineqgl > 0 & scbwineq == 1, vol_scwine := cwineqgl * volume_data[beverage == "winesglassvol", volume]]
    data[scwineq6 == 1 & !is.na(cwineqgl) & cwineqgl > 0 & scbwineq == 2, vol_scwine := cwineqgl * volume_data[beverage == "wineglassvol", volume]]
    data[scwineq6 == 1 & !is.na(cwineqgl) & cwineqgl > 0 & scbwineq == 3, vol_scwine := cwineqgl * volume_data[beverage == "winelglassvol", volume]]
  } else {
  ...
  if ((year %in% c(year_set2) & country == "England") | country == "Scotland") {
    ...
    data[, vol_scspirits := vol_scspirits * scspirit]
    data[, vol_scsherry := vol_scsherry * scsherry]
    data[, vol_scwine := vol_scwine * scwine]
    data[, vol_scpops := vol_scpops * scpops]


    # data[ , `:=`(scnbeer = NULL, scsbeer = NULL, scspirit = NULL, scsherry = NULL, scwine = NULL, scpops = NULL)]
  ...
  if (year %in% c(year_set1, year_set2) | country %in% c("Scotland", "Wales")) {
    ...
    if (year %in% c(year_set4) & country == "England") {
      ...
      data[, beer_units := rowSums(.SD, na.rm = TRUE), .SDcols = c("nbeer_units", "sbeer_units")]
      data[nbeer_units == 0 & sbeer_units == 0, beer_units := NA]
    }
    data[, wine_only_units := wine_units]
    data[, wine_units := rowSums(.SD, na.rm = TRUE), .SDcols = c("wine_units", "sherry_units")]
    data[wine_units == 0 & sherry_units == 0, wine_units := NA]
    ...
    if (year > 2019 & country == "Wales") {
      ...
      # protect NA if all are NA
      data[, weekmean := rowSums(.SD, na.rm = TRUE), .SDcols = c("spirit_units", "wine_units", "rtd_units", "beer_units")]
      data[is.na(spirit_units) & is.na(wine_units) & is.na(rtd_units) & is.na(beer_units), weekmean := NA]
    }
    ...
    data[perc_spirit_units > 0 & perc_spirit_units <= 50, spirits_pref_cat := "drinks_some_spirits"]
    data[perc_spirit_units > 50, spirits_pref_cat := "mostly_drinks_spirits"]

    data[perc_wine_units == 0, wine_pref_cat := "does_not_drink_wine"]
    data[perc_wine_units > 0 & perc_wine_units <= 50, wine_pref_cat := "drinks_some_wine"]
    data[perc_wine_units > 0.5, wine_pref_cat := "mostly_drinks_wine"]

    data[perc_rtd_units == 0, rtd_pref_cat := "does_not_drink_rtds"]
    data[perc_rtd_units > 0 & perc_rtd_units <= 50, rtd_pref_cat := "drinks_some_rtds"]
    data[perc_rtd_units > 50, rtd_pref_cat := "mostly_drinks_rtds"]

    data[perc_beer_units == 0, beer_pref_cat := "does_not_drink_beer"]
    data[perc_beer_units > 0 & perc_beer_units <= 50, beer_pref_cat := "drinks_some_beer"]
    data[perc_beer_units > 50, beer_pref_cat := "mostly_drinks_beer"]
  }
