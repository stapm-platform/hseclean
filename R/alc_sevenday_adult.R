...
alc_sevenday_adult <- function(
  ...
  if(!(country == "Wales" & year > 2019)){
    ...
    # HSE 2022 split cider into normal (<6% ABV) and strong (>=6% ABV) categories
    ...
    # Strong cider is defined as cider with ABV >= 6% (e.g., Frosty Jack's, White Lightning)
    ...
    if(is_hse_2022) {
      ...
      # Container types not used by a respondent are recorded as NA in the data
      # If we don't replace NA with 0, any NA * volume = NA, causing the entire calculation to fail
      data[is.na(scidqpt7), scidqpt7 := 0]
      data[is.na(scidsca7), scidsca7 := 0]
      data[is.na(scidlca7), scidlca7 := 0]
      data[is.na(scidbot7), scidbot7 := 0]

      data[ , d7vol_scider := 0]

      # Extract volumes before data.table operations to avoid scoping issues
      shalfvol <- alc_volume_data[beverage == "sciderhalfvol", volume]
      sscanvol <- alc_volume_data[beverage == "sciderscanvol", volume]
      slcanvol <- alc_volume_data[beverage == "sciderlcanvol", volume]
      sbtlvol <- alc_volume_data[beverage == "sciderbtlvol", volume]

      # Calculate total volume across all container types for strong cider drinkers (d7typ4 == 1)
      data[d7typ4 == 1, d7vol_scider := scidqpt7 * shalfvol * 2]  # Pints (using half-pint volume * 2)
      data[d7typ4 == 1, d7vol_scider := d7vol_scider + scidsca7 * sscanvol]  # Small cans
      data[d7typ4 == 1, d7vol_scider := d7vol_scider + scidlca7 * slcanvol]  # Large cans
      data[d7typ4 == 1, d7vol_scider := d7vol_scider + scidbot7 * sbtlvol]  # Bottles
    ...
    if(is_hse_2022) {
      data[ , `:=`(d7vol_nbeer = NULL, d7vol_sbeer = NULL, d7vol_ncider = NULL, d7vol_scider = NULL,
                   d7vol_spirits = NULL, d7vol_sherry = NULL, d7vol_wine = NULL, d7vol_pops = NULL)]
    } else {
      data[ , `:=`(d7vol_nbeer = NULL, d7vol_sbeer = NULL, d7vol_spirits = NULL,
