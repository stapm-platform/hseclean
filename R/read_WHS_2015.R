#' Read the Welsh Health Survey 2015 \lifecycle{maturing}
#'
#' Reads and does basic cleaning on the Welsh Health Survey 2015.
#'
...
#'
#' }
read_WHS_2015 <- function(
    root = c("X:/", "/Volumes/Shared/")[1],
    file = "HAR_PR/PR/Consumption_TA/HSE/Welsh Health Survey (WHS)/2015/UKDA-8090-tab/tab/whs_2015_adult_archive_v1.tab"
) {
  ...
  alc_vars <- Hmisc::Cs(
    ...
    alc6,                        # Max daily consumption last week (inc. heavy drinking)
    alcagbi,                     # Max daily consumption - above guidelines [binary]
    alcbibi,                     # Max daily consumption - binge [binary]
    alcvhbi                     # Very heavy drinking over 3 times guidelines
  )
  #
  ...
  smk_vars <- tolower(Hmisc::Cs(smok, smokec, smoked, smokee, smokstat, smokex, stpsmk, stpsmk1m, stpsmk1y))
  ...
  # Set PSU and cluster
  #data[ , psu := paste0("2015_", 1)]
  #data[ , cluster := paste0("2015_", 1)]
