...
alc_weekmean_adult <- function(
  ...
  if(year %in% c(year_set1, year_set2) | country %in% c("Scotland","Wales")) {
    ...
    # half pints
    ## NSW recorded in pints rather than half-pints after 2019-20 & HSE 2019 also
  ...
  if(year >= 2022 & country == "England") {
    ...
    data[ncidm221 == 1 & !is.na(ncid22a) & ncid22a > 0, vol_ncider := ncid22a * volume_data[beverage == "nciderpintvol", volume]]
    # Small cans
    data[ncidm222 == 1 & !is.na(ncid22b) & ncid22b > 0, vol_ncider := vol_ncider + ncid22b * volume_data[beverage == "nciderscanvol", volume]]
    # Large cans
    data[ncidm223 == 1 & !is.na(ncid22c) & ncid22c > 0, vol_ncider := vol_ncider + ncid22c * volume_data[beverage == "nciderlcanvol", volume]]
    # Bottles
    data[ncidm224 == 1 & !is.na(ncid22d) & ncid22d > 0, vol_ncider := vol_ncider + ncid22d * volume_data[beverage == "nciderbtlvol", volume]]

    # Handle "don't know" responses
    data[ncidm221 == 1 & ncid22a == -8, vol_ncider := NA]
    data[ncidm222 == 1 & ncid22b == -8, vol_ncider := NA]
    data[ncidm223 == 1 & ncid22c == -8, vol_ncider := NA]
    data[ncidm224 == 1 & ncid22d == -8, vol_ncider := NA]
  ...
  if(year >= 2022 & country == "England") {
    ...
    # Pints
    data[scidm221 == 1 & !is.na(scid22a) & scid22a > 0, vol_scider := scid22a * volume_data[beverage == "sciderpintvol", volume]]
    # Small cans
    data[scidm222 == 1 & !is.na(scid22b) & scid22b > 0, vol_scider := vol_scider + scid22b * volume_data[beverage == "sciderscanvol", volume]]
    # Large cans
    data[scidm223 == 1 & !is.na(scid22c) & scid22c > 0, vol_scider := vol_scider + scid22c * volume_data[beverage == "sciderlcanvol", volume]]
    # Bottles
    data[scidm224 == 1 & !is.na(scid22d) & scid22d > 0, vol_scider := vol_scider + scid22d * volume_data[beverage == "sciderbtlvol", volume]]

    # Handle "don't know" responses
    data[scidm221 == 1 & scid22a == -8, vol_scider := NA]
    data[scidm222 == 1 & scid22b == -8, vol_scider := NA]
    data[scidm223 == 1 & scid22c == -8, vol_scider := NA]
    data[scidm224 == 1 & scid22d == -8, vol_scider := NA]
